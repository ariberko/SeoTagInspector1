...
export default function NotFound() {
  ...
  return (
    <div className={`min-h-screen w-full flex items-center justify-center ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <Card className={`w-full max-w-md mx-4 ${theme === 'dark' ? 'border-gray-700' : ''}`}>
        <CardContent className="pt-6">
          <div className="flex mb-4 gap-2">
            <AlertCircle className="h-8 w-8 text-red-500" />
            <h1 className={`text-2xl font-bold ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>404 Page Not Found</h1>
          </div>
